						...
						if (UserRegistry.isPassword(p, password)) {
							do {
								...
								case "1":
									...
									if (!ServiceRegistry.addService(classeAjout)) {
										out.print(Decodage.encoder(TestBRi.validation(classeAjout)));
										urlcl = null;
										break;
									} else {
										out.print(Decodage.encoder("Classe, dans la norme BLTi "
												+ classeAjout.getSimpleName() + " ajoutée dans le ServiceRegistry\n"));
										urlcl = null;
										break;
									}
								...
								case "2":
									...
									if (!ServiceRegistry.replaceService(classeUpdate)) {
										out.print(Decodage.encoder("Aucune mise à jour effectuée\n"));
										urlcl = null;
										break;
								...
								case "4":
									...
									case "1":
										...
										Class<? extends Service> classeStart = ServiceRegistry
												.getServiceClass(choixDem);

										if (!p.getLogin().equals(classeStart.getName().substring(0,
												classeStart.getName().indexOf(".")))) {
											out.print(Decodage
													.encoder("Le service à démarrer ne vous appartient pas \n"));
											break;
										}
									...
									case "2":
										...
										Class<? extends Service> classeStop = ServiceRegistry
												.getServiceClass(choixArret);

										if (!p.getLogin().equals(
												classeStop.getName().substring(0, classeStop.getName().indexOf(".")))) {
											out.print(
													Decodage.encoder("Le service à arrêter ne vous appartient pas \n"));
											break;
										}
								...
								case "5":
									...
									Class<? extends Service> classeDesinstall = ServiceRegistry
											.getServiceClass(choixDesinstall);

									if (!p.getLogin().equals(classeDesinstall.getName().substring(0,
											classeDesinstall.getName().indexOf(".")))) {
										out.print(Decodage
												.encoder("Le service à désinstaller ne vous appartient pas \n"));
										break;
									}
									if (ServiceRegistry
											.removeService(ServiceRegistry.getServiceClass(choixDesinstall))) {
										out.print(Decodage.encoder(
												"Désinstallation effectuée, le service n'est plus dans le ServiceRegistry\n"));
										break;
									}
