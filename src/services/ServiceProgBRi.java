				...
				if (ProgrammerRegistry.containsLogin(login) != null) {
					...
					if (ProgrammerRegistry.isPassword(p, password)) {
						do {
							...
							case "1":
								out.println(
										Decodage.encoder("Donnez le nom de la classe à ajouter (plus son package):"));
								...
								out.print(Decodage
										.encoder("Classe " + c.getSimpleName() + " ajoutée dans le ServiceRegistry\n"));
							...
							// Fin du service
							out.println(Decodage.encoder("Voulez vous arreter ou vous déconnecter ? (stop/deco/caractère quelconque)"));
